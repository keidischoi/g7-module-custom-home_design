...
- 상품 썸네일을 더 줄여 태블릿·데스크톱에서 한 줄에 6개가 들어가게 합니다.
- 카테고리 `전체`의 최근 본 상품, 인기상품, 신상품 가로 스크롤도 같은 크기로 맞춥니다.

## [0.2.32] - 2026-09-11

### Fixed — 상품 썸네일이 실제로 작아지게 수정

- 열 수를 모바일 3 / 태블릿 4 / 데스크톱 5로 늘려 썸네일 가로 크기를 줄입니다.
- 쇼핑 목록에서는 카드 클래스 없이도 바로 적용되게 했습니다.
- 할인율 글씨는 작아진 썸네일에 맞게 다시 맞춥니다.

## [0.2.31] - 2026-09-11

### Changed — 쇼핑 상품 썸네일을 번개장터 비율로 조정

- 상품 썸네일을 번개장터와 같은 81:100 세로형으로 맞춥니다.
- 모바일 2열 / 데스크톱 4열로 두고, 간격과 모서리를 더 촘촘하게 맞춥니다.
- 썸네일 안 할인율 글씨는 새 크기에 맞게 좌상단으로 다시 맞춥니다.

## [0.2.30] - 2026-09-11

### Changed — 쇼핑 목록 버튼을 오른쪽 분리

- 카테고리 칩과 모든 상품 / 최근 본 상품 / 인기상품 / 신상품을 한 줄에 섞지
  않고, 목록 버튼만 오른쪽에 따로 둡니다.
...
- 카테고리 `전체`에서는 최근 본 상품, 인기상품, 신상품 섹션과 기존 페이지네이션을
  다시 보여 줍니다.
- 모든 상품 / 최근 본 상품 / 인기상품 / 신상품 버튼을 눌렀을 때만 해당 목록
  그리드와 무한 스크롤을 사용합니다.
...
- 쇼핑 페이지 카테고리 버튼 옆에 인기상품, 모든 상품, 최근 본 상품, 신상품을
  이벤트 훅으로 추가합니다.
- 선택한 목록만 기존 상품 썸네일 그리드에 표시하고, 스크롤 시 다음 페이지를
  이어서 불러옵니다. 최근 본 상품은 저장된 목록만 보여 줍니다.

## [0.2.24] - 2026-09-11

### Fixed — 로그인·회원가입 박스 폭 복원

- 콘텐츠 최대 폭 패치가 `max-w-md` 폼 카드까지 페이지 폭으로 늘리던 문제를
  수정했습니다.
- 로그인·회원가입·비밀번호 찾기 등 인증 페이지는 원래 카드 크기를 유지합니다.

## [0.2.23] - 2026-09-11

### Fixed — 인기·쇼핑 페이지 이동 시 헤더 가로 이동

- 페이지마다 세로 스크롤바 유무가 달라지면서 뷰포트 폭과 중앙 정렬 기준이 바뀌던
  문제를 수정했습니다.
- 브라우저가 스크롤바 자리를 항상 확보하도록 해 인기·쇼핑 및 다른 메뉴로 이동해도
  상단 헤더와 메뉴의 가로 위치를 유지합니다.

## [0.2.22] - 2026-09-11

### Fixed — 쇼핑 이동 시 상단 메뉴 위치 이동

- 비쇼핑 페이지에서 통화 선택기를 DOM에서 제거해 우측 액션 영역 폭이 달라지던 문제를
  수정했습니다.
- 데스크톱에서는 통화 선택기 자리를 유지한 채 숨기고, 쇼핑 페이지에서 같은 자리로
  표시해 나머지 상단 아이콘 위치가 움직이지 않습니다.
- 모바일 드로어에서는 빈 공간이 생기지 않도록 기존처럼 완전히 숨깁니다.
...
- 홈·게시판 등 비쇼핑 페이지 헤더에서 통화 아이콘을 숨깁니다.
- 쇼핑 레이아웃(`/shop`, 장바구니/결제, 주문·위시리스트·배송지)에서는 기존처럼 표시합니다.
- SPA 이동 시에도 경로 기준으로 다시 맞춥니다.
