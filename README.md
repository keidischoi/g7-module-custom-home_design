...
- **Header icon packing**: 아이콘이 가득 찬 뒤 하나가 사라져도 안쪽만 가리지 않고
  헤더 우측 flex 칸 자체를 접어, 옆 버튼이 빈 자리를 메웁니다.

## 0.2.35 notes

- **Hidden header icons**: 비쇼핑 페이지에서 통화 선택기를 숨길 때 자리만 남기지 않고
  공백까지 접어서, 장바구니·언어·프로필 아이콘이 붙습니다.

## 0.2.34 notes

- **Faster load**: 심긴 설정을 그대로 쓰고, 중복 DOM 작업과 설정 API 호출을 줄입니다.

## 0.2.33 notes

- **Shop thumbnails**: 한 줄에 6개(모바일은 3열)가 들어가게 더 줄이고,
  전체 카테고리의 최근 본 / 인기 / 신상품 섹션도 같은 크기로 맞춥니다.

## 0.2.32 notes

- **Shop thumbnails**: 상품 썸네일 가로 크기를 줄입니다. 모바일 3열, 태블릿 4열,
  데스크톱 5열이며 할인율 글씨도 함께 맞춥니다.

## 0.2.31 notes

- **Shop thumbnails**: 상품 카드를 번개장터처럼 81:100 세로 썸네일로 맞추고,
  안에 있는 할인율 글씨 위치도 함께 조정합니다.
