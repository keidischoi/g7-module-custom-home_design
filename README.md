...
- **Favicon upload**: 관리자 설정에서 파일 선택 즉시 업로드·저장되고 미리보기가 보입니다.
- **Admin UI**: 설정 화면을 섹션 카드와 토글로 정리했습니다.

## 0.2.35 notes

- **Header packing**: 비쇼핑 페이지에서 통화 칸만 `display:none`으로 접어
  옆 아이콘이 빈 자리로 붙습니다. 공유 메뉴는 건드리지 않습니다.
